...
watch(() => data.value.banks, (banks) => {
  if (!routeSelectionApplied.value && banks.length) {
    const requested = Array.isArray(route.query.bank) ? route.query.bank[0] : route.query.bank
    if (typeof requested === 'string' && banks.some(bank => bank.id === requested)) {
...
<template>
  <CrmShell title="Instytucje finansowe" eyebrow="SuperAdmin · katalog instytucji">
    <template #actions>
      <UButton :to="`/org/${organizationSlug}/mortgages/offers`" icon="i-lucide-badge-percent">Oferty kredytowe</UButton>
    ...
    <template v-else>
      <section class="admin-notice">
        ...
        <div>
          ...
          <p>Możesz zmienić nazwę, stronę i logo instytucji albo ukryć wszystkie jej oferty w porównywarce.</p>
        </div>
      </section>
        ...
        <aside class="institution-list">
          ...
          >
            ...
            <span class="institution-item__copy">
              ...
              <small>{{ bank.productCount }} {{ bank.productCount === 1 ? 'oferta' : 'ofert' }}</small>
            </span>
        ...
        <main v-if="selected" class="editor">
          <form @submit.prevent="save">
            <UCard>
              ...
              <div class="form-grid">
                <UFormField
                  ...
                  description="Wyłączenie ukrywa wszystkie oferty tej instytucji w organizacji."
                >
            ...
            <div class="sticky-actions">
              <div>
                <strong>{{ selected.productCount }}</strong> {{ selected.productCount === 1 ? 'aktywna oferta' : 'aktywnych ofert' }}
...
.card-head__actions { display: flex; align-items: center; flex-wrap: wrap; justify-content: flex-end; gap: 8px; }
...
@media (max-width: 700px) { .institution-list, .form-grid, .logo-editor { grid-template-columns: 1fr; } .card-head { flex-direction: column; } .card-head__actions { justify-content: flex-start; } .sticky-actions { align-items: stretch; flex-direction: column; } .sticky-actions > div { margin-right: 0; } }
</style>
