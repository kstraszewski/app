...
const registeredPdfUrl = computed(() => props.sourceKind === 'registered'
  ? `/api/multiform/demo-pdfs/${encodeURIComponent(props.templateId)}`
  : '')
const effectivePdfUrl = computed(() => localPdfUrl.value || registeredPdfUrl.value)

watch(() => props.templateText, parseTemplateText, { immediate: true })
watch([pageNumber, zoom], () => { void renderCurrentPage() })
watch(() => [props.templateId, props.sourceKind], () => {
  releaseLocalPdf()
  undoStack.value = []
  redoStack.value = []
...
<style scoped>
.visual-editor { min-height: 640px; background: var(--bg-subtle); }
.visual-toolbar { min-height: 45px; display: flex; align-items: center; justify-content: space-between; gap: 12px; padding: 7px 10px; background: var(--bg-default); border-bottom: 1px solid var(--border-default); }
.visual-toolbar__group { display: flex; align-items: center; gap: 6px; }
.tool-button,
.page-button,
.attach-button { min-height: 30px; padding: 6px 9px; color: var(--fg-secondary); background: var(--bg-default); border: 1px solid var(--border-default); border-radius: var(--radius-sm); font: inherit; font-size: 10px; cursor: pointer; }
.tool-button:hover:not(:disabled), .page-button:hover:not(:disabled), .attach-button:hover { color: var(--fg-primary); border-color: var(--border-strong); }
.tool-button--active { color: #fff; background: var(--admin-accent); border-color: var(--admin-accent); }
.tool-button:disabled, .page-button:disabled { opacity: .45; cursor: default; }
.toolbar-check,
.zoom-control { display: inline-flex; align-items: center; gap: 6px; color: var(--fg-tertiary); font-family: var(--font-mono); font-size: 9px; }
.zoom-control select,
.pdf-pane__header select { min-height: 29px; color: var(--fg-primary); background: var(--bg-default); border: 1px solid var(--border-default); border-radius: var(--radius-sm); font: inherit; font-size: 10px; }
.visual-error,
.visual-notice { padding: 9px 12px; margin: 0; border-bottom: 1px solid; font-size: 10px; }
.visual-error { color: var(--admin-danger); background: var(--admin-danger-soft); border-color: color-mix(in srgb, var(--admin-danger) 25%, transparent); }
.visual-notice { color: var(--admin-positive); background: var(--admin-positive-soft); border-color: color-mix(in srgb, var(--admin-positive) 25%, transparent); }

.visual-layout { min-width: 880px; display: grid; grid-template-columns: 190px minmax(390px, 1fr) 225px; min-height: 650px; }
.field-browser,
.property-pane { min-width: 0; background: var(--bg-default); }
.field-browser { border-right: 1px solid var(--border-default); }
.property-pane { border-left: 1px solid var(--border-default); }
.pane-heading { min-height: 43px; display: flex; align-items: center; justify-content: space-between; padding: 9px 11px; border-bottom: 1px solid var(--border-default); }
.pane-heading span { color: var(--fg-tertiary); font-family: var(--font-mono); font-size: 9px; letter-spacing: .08em; text-transform: uppercase; }
.pane-heading strong { padding: 3px 6px; color: var(--admin-accent); background: var(--admin-accent-soft); border-radius: 999px; font-family: var(--font-mono); font-size: 8px; }
.field-search { display: block; padding: 9px; }
.field-search input { width: 100%; height: 34px; padding: 7px 9px; color: var(--fg-primary); background: var(--bg-subtle); border: 1px solid var(--border-default); border-radius: var(--radius-sm); font: inherit; font-size: 10px; }
.field-filters { display: flex; gap: 4px; padding: 0 9px 9px; }
.field-filters button { padding: 4px 6px; color: var(--fg-tertiary); background: transparent; border: 1px solid var(--border-default); border-radius: 999px; font: inherit; font-size: 8px; cursor: pointer; }
.field-filters button.active { color: var(--admin-accent); background: var(--admin-accent-soft); border-color: color-mix(in srgb, var(--admin-accent) 35%, transparent); }
.field-list { max-height: 565px; overflow: auto; padding: 0 7px 10px; }
.field-list-item { position: relative; width: 100%; display: flex; align-items: center; gap: 6px; padding: 8px; margin-bottom: 4px; color: var(--fg-primary); text-align: left; background: transparent; border: 1px solid transparent; border-radius: var(--radius-sm); font: inherit; cursor: pointer; }
.field-list-item:hover { background: var(--bg-subtle); border-color: var(--border-default); }
.field-list-item--selected { background: var(--admin-accent-soft); border-color: color-mix(in srgb, var(--admin-accent) 35%, transparent); }
...
.field-list-item small { color: var(--fg-tertiary); font-size: 8px; }
.field-page { color: var(--fg-tertiary); font-family: var(--font-mono); font-size: 8px; white-space: nowrap; }
.override-dot { width: 17px; height: 17px; display: grid; place-items: center; color: #fff; background: var(--admin-warning); border-radius: 50%; font-family: var(--font-mono); font-size: 7px; }

.pdf-pane { min-width: 0; display: flex; flex-direction: column; background: #dce2ea; }
.pdf-pane__header { min-height: 43px; display: flex; align-items: center; justify-content: center; gap: 7px; color: var(--fg-secondary); background: var(--bg-default); border-bottom: 1px solid var(--border-default); font-size: 9px; }
.pdf-pane__header label { display: inline-flex; align-items: center; gap: 5px; }
.page-button { width: 30px; padding: 5px; }
.page-field-count { margin-left: 7px; padding-left: 9px; color: var(--fg-tertiary); border-left: 1px solid var(--border-default); font-family: var(--font-mono); }
...
.field-box:focus-visible { box-shadow: 0 0 0 3px #fff, 0 0 0 5px var(--admin-accent); }
...
.pdf-message { max-width: 420px; align-self: center; padding: 28px; margin: 70px 20px; color: #334155; text-align: center; background: rgb(255 255 255 / 75%); border: 1px solid #cbd5e1; border-radius: var(--radius-md); }
.pdf-message strong { display: block; font-size: 13px; }
.pdf-message p { margin: 7px 0 14px; font-size: 10px; line-height: 1.5; }

.property-title { display: grid; gap: 4px; padding: 11px; border-bottom: 1px solid var(--border-default); }
.property-title strong { overflow-wrap: anywhere; font-family: var(--font-mono); font-size: 9px; line-height: 1.45; }
.property-title span { color: var(--fg-tertiary); font-size: 8px; }
.property-pane fieldset { display: grid; gap: 8px; padding: 11px; margin: 0; border: 0; border-bottom: 1px solid var(--border-default); }
.property-pane fieldset:disabled { opacity: .58; }
.property-pane legend { float: left; width: 100%; padding: 0 0 8px; color: var(--fg-tertiary); font-family: var(--font-mono); font-size: 8px; letter-spacing: .07em; text-transform: uppercase; }
.property-pane label { display: grid; gap: 4px; color: var(--fg-tertiary); font-size: 8px; }
.property-pane input,
.property-pane select { width: 100%; min-height: 31px; padding: 6px 7px; color: var(--fg-primary); background: var(--bg-subtle); border: 1px solid var(--border-default); border-radius: var(--radius-sm); font: inherit; font-size: 9px; }
.number-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 7px; }
.property-hint,
.keyboard-tip { margin: 0; color: var(--fg-tertiary); font-size: 8px; line-height: 1.5; }
.reset-placement { width: calc(100% - 22px); padding: 8px; margin: 11px; color: var(--admin-warning); background: var(--admin-warning-soft); border: 1px solid color-mix(in srgb, var(--admin-warning) 30%, transparent); border-radius: var(--radius-sm); font: inherit; font-size: 9px; cursor: pointer; }
.reset-placement:disabled { opacity: .5; cursor: default; }
.keyboard-tip { padding: 0 11px 11px; }
.empty-pane { padding: 16px 10px; margin: 0; color: var(--fg-tertiary); font-size: 9px; line-height: 1.5; }
.empty-pane--properties { padding: 24px 14px; text-align: center; }
.empty-pane--properties strong { display: block; margin-bottom: 5px; color: var(--fg-primary); font-size: 11px; }
