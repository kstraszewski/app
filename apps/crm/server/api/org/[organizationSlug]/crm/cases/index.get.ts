...
export default defineEventHandler(async (event) => {
  ...
  const [clientsResult, itemsResult] = await Promise.all([
    clientIds.length
      ? session.supabase.from('crm_clients').select('id, display_name, status_code, primary_email, primary_phone').eq('organization_id', session.organizationId).in('id', clientIds)
      : Promise.resolve({ data: [], error: null }),
    caseIds.length
      ? session.supabase.from('crm_case_items').select('id, case_id, product_type_id, title, status_code, amount_value, currency, updated_at').eq('organization_id', session.organizationId).in('case_id', caseIds)
      : Promise.resolve({ data: [], error: null }),
  ])
