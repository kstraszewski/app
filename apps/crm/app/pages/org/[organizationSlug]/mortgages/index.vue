...
type CatalogProduct = {
  ...
  slug: string
  name: string
  bank: { slug: string, name: string, website_url: string, logoUrl: string | null, logoBackground: string | null }
  version: {
    version_key: string
    retrieved_at: string
...
<template>
  <CrmShell title="Porównywarka hipotek" :eyebrow="`Kalkulacja orientacyjna · Polska · ${banks.length} instytucji`">
    <template #actions>
      <UButton v-if="data.role === 'admin'" :to="`/org/${organizationSlug}/mortgages/admin`" icon="i-lucide-package-open" variant="outline">Produkty</UButton>
      <UButton v-if="data.role === 'admin'" :to="`/org/${organizationSlug}/mortgages/institutions`" icon="i-lucide-landmark" variant="outline">Instytucje</UButton>
