...
export function readCornerRadius(el: HTMLElement): number {
  ...
  // The current renderer accepts one uniform radius. Use the most
  // conservative corner so a square corner is never painted as rounded.
...
export function readHairlineSpec(el: HTMLElement): { width: number; outerCssPx: number } {
  ...
  if (shadow && shadow !== 'none') {
    ...
    for (const part of parts) {
      ...
      const spreadValue = nums[3];
      if (spreadValue === undefined) continue;
...
export function attachObservers(t: ReflectionTarget): void {
  if (typeof ResizeObserver !== 'undefined') {
    t.resizeObserver = new ResizeObserver(() => {
      refreshTargetStyles(t);
      t.requestPaint();
    });
    t.resizeObserver.observe(t.el);
  }
  if (typeof MutationObserver !== 'undefined') {
    t.mutationObserver = new MutationObserver(() => {
      refreshTargetStyles(t);
      t.requestPaint();
    });
