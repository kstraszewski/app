...
function parseClaimedReanalysis(value: unknown): ClaimedReanalysis {
  ...
  if (
    typeof result.shouldDispatch !== 'boolean'
    || typeof result.replayed !== 'boolean'
  ) {
    throw new Error('Invalid bank-mail reanalysis dispatch flags.')
  }
...
export async function dispatchBankMailReanalysisWithDependencies(
  ...
  const recordDispatchFailure = async (): Promise<void> => {
    await rpc(
      dependencies,
  ...
  try {
    ...
    try {
      ...
      // Preserve the dispatch failure; stale-request recovery remains a DB concern.
    }
    throw error
  }
}
