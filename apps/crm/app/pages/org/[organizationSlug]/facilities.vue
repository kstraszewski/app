...
type FacilityWorkspace = {
  ...
  appointments: FacilityAppointmentsPayload
}
...
const selectedFacilityId = ref(typeof route.query.facility === 'string' ? route.query.facility : '')
...
const sectionItems = [
  ...
  { label: 'Eksperci i usługi', value: 'experts', icon: 'i-lucide-briefcase-business' },
...
const roleItems = [
  { label: 'Administrator placówki', value: 'admin' },
...
watch(() => facilitiesPayload.value.data, (facilities) => {
  if (selectedFacilityId.value && facilities.some(facility => facility.id === selectedFacilityId.value)) return
  const requested = typeof route.query.facility === 'string' ? route.query.facility : ''
  selectedFacilityId.value = facilities.find(facility => facility.id === requested)?.id ?? facilities[0]?.id ?? ''
}, { immediate: true })
...
const { data: workspace, status: workspaceStatus, error: workspaceError, refresh: refreshWorkspace } = await useLazyAsyncData<FacilityWorkspace>(
  ...
  async () => {
    ...
    const [detail, members, teamLinks, hours, services, widgets, appointments, facilityCalendar, expertCalendar] = await Promise.all([
      $fetch<FacilityDetailPayload>(base),
      $fetch<FacilityMembersPayload>(`${base}/members`),
      $fetch<FacilityTeamLinksPayload>(`${base}/team-links`),
      $fetch<FacilityHoursPayload>(`${base}/hours`),
      $fetch<FacilityServicesPayload>(`${base}/services`),
      $fetch<FacilityWidgetsPayload>(`${base}/widgets`),
      $fetch<FacilityAppointmentsPayload>(orgApiPath('/appointments'), {
        query: {
          facilityId,
          status: appointmentStatus.value === 'all' ? undefined : appointmentStatus.value,
          expertUserId: appointmentExpertId.value || undefined,
    ...
    return { detail, members, teamLinks, hours, services, widgets, appointments, facilityCalendar, expertCalendar }
  },
  {
    server: false,
...
const isOrganizationAdmin = computed(() => facilitiesPayload.value.role === 'admin')
...
const catalogServiceItems = computed(() => {
  const assigned = new Set(workspace.value.services.data.map(service => service.id))
  return [
    ...(isOrganizationAdmin.value ? [{ label: 'Utwórz nową usługę', value: '' }] : []),
    ...workspace.value.services.catalog
...
const staffAppointmentServiceItems = computed(() => staffAppointmentServices.value.map(service => ({
...
const serviceRows = computed(() => workspace.value.services.data.map(service => ({
  service,
  draft: serviceDrafts[service.id] ?? serviceToDraft(service),
})))
...
const filteredAppointments = computed(() => workspace.value.appointments.data)
const appointmentPageStart = computed(() => workspace.value.appointments.count
...
const appointmentPageEnd = computed(() => Math.min(
  ...
  workspace.value.appointments.count,
))
...
watch(() => workspace.value.services.data, (services) => {
  for (const key of Object.keys(serviceDrafts)) delete serviceDrafts[key]
  for (const service of services) serviceDrafts[service.id] = serviceToDraft(service)
}, { immediate: true })
...
watch(() => widgetForm.fixedExpertUserId, (fixedExpertUserId) => {
  ...
  widgetForm.serviceIds = widgetForm.serviceIds.filter(serviceId => allowedServiceIds.has(serviceId))
})
...
async function loadCalendarState(ownerKind: 'facility' | 'expert', ownerId: string): Promise<CalendarState> {
  try {
    const data = await $fetch<FacilityCalendarConnectionsPayload>(orgApiPath('/calendar-connections'), {
...
function serviceToDraft(service: BookingService): ServiceDraft {
  return {
    name: service.name,
    ...
    expertUserIds: service.expertUserIds.filter(userId => (
      bookableExperts.value.some(member => member.user_id === userId)
    )),
  }
}
...
function setIdSelected(values: string[], id: string, selected: boolean | 'indeterminate') {
  const index = values.indexOf(id)
  if (selected === true && index === -1) values.push(id)
  if (selected !== true && index >= 0) values.splice(index, 1)
}

function isIdSelected(values: string[], id: string) {
...
async function createService() {
  if (!canManage.value || !selectedFacilityId.value) return
  if (!serviceForm.existingServiceId && (!isOrganizationAdmin.value || !serviceForm.name.trim())) return
  savingService.value = true
  try {
    const body = serviceForm.existingServiceId
      ? { serviceId: serviceForm.existingServiceId, expertUserIds: serviceForm.expertUserIds, isAvailable: true }
      : {
          ...
          slotIntervalMinutes: serviceForm.slotIntervalMinutes,
          minNoticeMinutes: serviceForm.minNoticeMinutes,
          maxAdvanceDays: serviceForm.maxAdvanceDays,
          isActive: serviceForm.isActive,
          isAvailable: true,
          expertUserIds: serviceForm.expertUserIds,
        }
    await $fetch(orgApiPath(`/facilities/${encodeURIComponent(selectedFacilityId.value)}/services`), {
      method: 'POST',
      body,
    })
    ...
    await refreshWorkspace()
    toast.add({ title: 'Usługa została udostępniona w placówce', color: 'success' })
  } catch (error: unknown) {
    showActionError('Nie udało się zapisać usługi', error)
  } finally {
...
async function saveService(serviceId: string) {
  ...
  try {
    const body: Record<string, unknown> = {
      ...
      expertUserIds: draft.expertUserIds,
    }
    if (isOrganizationAdmin.value) Object.assign(body, draft)
    await $fetch(orgApiPath(`/facilities/${encodeURIComponent(selectedFacilityId.value)}/services/${encodeURIComponent(serviceId)}`), {
      method: 'PATCH',
      body,
    })
    await refreshWorkspace()
    toast.add({ title: 'Usługa została zapisana', color: 'success' })
  } catch (error: unknown) {
    showActionError('Nie udało się zapisać usługi', error)
  } finally {
    savingService.value = false
    ...
    const result = await $fetch<{ data: BookingWidget }>(
      orgApiPath(`/facilities/${encodeURIComponent(selectedFacilityId.value)}/widgets`),
      {
        ...
        body: {
          ...
          serviceIds: widgetForm.serviceIds.filter(serviceId => allowedServiceIds.has(serviceId)),
    ...
    await $fetch(
      orgApiPath(`/facilities/${encodeURIComponent(selectedFacilityId.value)}/widgets/${encodeURIComponent(selectedWidget.value.id)}`),
      {
        ...
        body: {
          ...
          serviceIds: widgetDraft.serviceIds.filter(serviceId => allowedServiceIds.has(serviceId)),
...
function connectCalendar(
  ...
  const returnTo = `/org/${organizationSlug.value}/facilities?facility=${selectedFacilityId.value}&section=calendars`
...
<template>
  <CrmShell title="Placówki" eyebrow="Umawianie spotkań · dostępność · integracje kalendarzy">
    <template #actions>
      <UButton
        v-if="facilitiesPayload.canCreate"
        icon="i-lucide-plus"
    ...
    <div class="facility-layout">
      <aside class="facility-sidebar">
        <div class="facility-sidebar__head">
          <div>
            <p class="section-kicker">Placówki</p>
            <strong>{{ facilities.length }}</strong>
          </div>
          <UBadge :color="facilitiesPayload.canCreate ? 'primary' : 'neutral'" variant="subtle">
            {{ facilitiesPayload.canCreate ? 'Administrator' : 'Ekspert' }}
          </UBadge>
        </div>
        ...
        <UInput
          ...
          aria-label="Szukaj placówki"
        />

        <div v-if="facilitiesStatus === 'pending'" class="facility-list" aria-label="Ładowanie placówek">
          <USkeleton v-for="index in 4" :key="index" class="h-20 w-full" />
        </div>

        <div v-else-if="visibleFacilities.length" class="facility-list">
          <button
            v-for="facility in visibleFacilities"
            :key="facility.id"
            type="button"
            class="facility-list-item"
            :class="{ 'facility-list-item--active': facility.id === selectedFacilityId }"
            @click="selectFacility(facility.id)"
          >
            <span class="facility-list-item__icon">
              <UIcon name="i-lucide-building-2" />
            </span>
            <span class="facility-list-item__body">
              <strong>{{ facility.name }}</strong>
              <small>{{ facility.city || facilityAddress(facility) || facility.timezone }}</small>
            </span>
            <span class="facility-list-item__status" :class="{ 'facility-list-item__status--off': !facility.is_active }" />
          </button>
        </div>

        <div v-else class="empty-compact">
          <UIcon name="i-lucide-building-2" />
          <p>{{ facilities.length ? 'Brak placówek pasujących do wyszukiwania.' : 'Nie masz jeszcze żadnej placówki.' }}</p>
          <UButton
            v-if="facilitiesPayload.canCreate && !facilities.length"
            size="sm"
            variant="outline"
            icon="i-lucide-plus"
            @click="openCreateFacility"
      ...
      <section class="facility-workspace">
        <div v-if="!selectedFacilityId" class="workspace-empty">
          ...
          <p>Po wybraniu skonfigurujesz jej zespół, dostępność, usługi, widget rezerwacji i kalendarze.</p>
        </div>
        ...
        <template v-else-if="selectedFacility">
          ...
          <section v-else-if="activeSection === 'experts'" class="workspace-section">
            <div class="section-heading">
              <div>
                ...
                <h3>Grafiki ekspertów i usługi</h3>
            ...
            <article class="panel-card panel-card--wide">
              ...
              <template v-if="selectedExpertId">
                ...
                <div v-if="canEditSelectedExpert" class="form-actions">
                  <UButton icon="i-lucide-save" :loading="savingExpertSchedule" @click="saveExpertSchedule">
                    Zapisz grafik eksperta
                  </UButton>
                </div>
              </template>
            </article>

            <div class="subsection-heading subsection-heading--actions">
              <div>
                <h4>Usługi dostępne w placówce</h4>
                <p>Określ czas trwania, reguły rezerwacji i ekspertów świadczących każdą usługę.</p>
              </div>
              <UButton v-if="canManage" icon="i-lucide-plus" @click="openCreateService">
                Dodaj usługę
              </UButton>
            </div>

            <UAlert
              v-if="canManage && !isOrganizationAdmin"
              color="neutral"
              variant="subtle"
              icon="i-lucide-info"
              title="Zakres uprawnień administratora placówki"
              description="Możesz włączać usługę i przypisywać ekspertów. Parametry wspólnego katalogu zmienia administrator organizacji."
            />

            <div v-if="serviceRows.length" class="service-list">
              <article v-for="row in serviceRows" :key="row.service.id" class="service-card">
                <div class="service-card__head">
                  <div>
                    <UBadge :color="row.draft.isAvailable && row.draft.isActive ? 'success' : 'neutral'" variant="subtle">
                      {{ row.draft.isAvailable ? 'Dostępna w placówce' : 'Wyłączona w placówce' }}
                    </UBadge>
                    <h4>{{ row.service.name }}</h4>
                  </div>
                  <USwitch v-model="row.draft.isAvailable" :disabled="!canManage" aria-label="Usługa dostępna w placówce" />
                </div>

                <div class="form-grid form-grid--two">
                  <UFormField :name="`serviceName-${row.service.id}`" label="Nazwa">
                    <UInput v-model="row.draft.name" class="w-full" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                  <UFormField :name="`serviceDuration-${row.service.id}`" label="Czas trwania (min)">
                    <UInputNumber
                      v-model="row.draft.durationMinutes"
                      :min="5"
                      :max="1440"
                      class="w-full"
                      :disabled="!canManage || !isOrganizationAdmin"
                    />
                  </UFormField>
                </div>
                <UFormField :name="`serviceDescription-${row.service.id}`" label="Opis">
                  <UTextarea v-model="row.draft.description" class="w-full" :rows="2" :disabled="!canManage || !isOrganizationAdmin" />
                </UFormField>

                <div class="number-grid">
                  <UFormField :name="`serviceSlot-${row.service.id}`" label="Interwał slotów">
                    <UInputNumber v-model="row.draft.slotIntervalMinutes" :min="5" :max="240" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                  <UFormField :name="`serviceBefore-${row.service.id}`" label="Bufor przed">
                    <UInputNumber v-model="row.draft.bufferBeforeMinutes" :min="0" :max="240" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                  <UFormField :name="`serviceAfter-${row.service.id}`" label="Bufor po">
                    <UInputNumber v-model="row.draft.bufferAfterMinutes" :min="0" :max="240" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                  <UFormField :name="`serviceNotice-${row.service.id}`" label="Wyprzedzenie min. (min)">
                    <UInputNumber v-model="row.draft.minNoticeMinutes" :min="0" :max="10080" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                  <UFormField :name="`serviceAdvance-${row.service.id}`" label="Horyzont (dni)">
                    <UInputNumber v-model="row.draft.maxAdvanceDays" :min="1" :max="730" :disabled="!canManage || !isOrganizationAdmin" />
                  </UFormField>
                </div>

                <div class="assignment-block">
                  <strong>Eksperci świadczący usługę</strong>
                  <div v-if="bookableExperts.length" class="checkbox-grid">
                    <UCheckbox
                      v-for="expert in bookableExperts"
                      :key="expert.user_id"
                      :model-value="isIdSelected(row.draft.expertUserIds, expert.user_id)"
                      :label="facilityMemberLabel(expert)"
                      :disabled="!canManage"
                      @update:model-value="value => setIdSelected(row.draft.expertUserIds, expert.user_id, value)"
                    />
                  </div>
                  <p v-else class="empty-line">Brak ekspertów przyjmujących rezerwacje.</p>
                </div>
                ...
                <div v-if="canManage" class="form-actions">
                  <UButton
                    ...
                    @click="saveService(row.service.id)"
                  >
                    Zapisz usługę
                  </UButton>
                </div>
            ...
            <div v-if="!filteredAppointments.length" class="workspace-empty workspace-empty--small">
              <UIcon name="i-lucide-briefcase-business" />
              <p>Nie udostępniono jeszcze usług w tej placówce.</p>
            </div>
          </section>
          ...
          <section v-else-if="activeSection === 'widget'" class="workspace-section">
            ...
            <UAlert
              ...
              description="Widget będzie automatycznie przypisany do Ciebie i pokaże wyłącznie terminy usług, które świadczysz w tej placówce."
            />
            ...
            <div v-if="workspace.widgets.data.length" class="widget-layout">
              <div class="form-stack">
                ...
                <template v-if="selectedWidget">
                  ...
                  <div class="assignment-block">
                    <strong>Usługi widoczne w widgecie</strong>
                    <div v-if="widgetServicesForEditor.length" class="checkbox-grid">
                      <UCheckbox
                        v-for="service in widgetServicesForEditor"
                        :key="service.id"
                        :model-value="isIdSelected(widgetDraft.serviceIds, service.id)"
                        :label="service.name"
                        :disabled="!canEditSelectedWidget"
                        @update:model-value="value => setIdSelected(widgetDraft.serviceIds, service.id, value)"
                      />
                    </div>
                    <p v-else class="empty-line">Najpierw udostępnij aktywną usługę w placówce.</p>
                  </div>
          ...
          <section v-else-if="activeSection === 'appointments'" class="workspace-section">
            <div class="section-heading section-heading--actions">
              <div>
                ...
                <p>{{ workspace.appointments.count }} wizyt dla bieżących filtrów.</p>
              </div>
              <div class="appointment-filters">
                ...
                <USelect
                  ...
                  :items="[{ label: 'Wszyscy eksperci', value: '' }, ...expertItems]"
                  value-key="value"
            ...
            <div v-if="workspace.appointments.count > appointmentPageSize" class="pagination-row">
              <span>Pozycje {{ appointmentPageStart }}–{{ appointmentPageEnd }} z {{ workspace.appointments.count }}</span>
              <div>
                <UButton
                ...
                <UButton
                  ...
                  color="neutral"
                  variant="outline"
                  :disabled="appointmentPageEnd >= workspace.appointments.count"
                  @click="changeAppointmentPage(1)"
                >Następne</UButton>
              </div>
            </div>
    ...
    <UModal
      ...
      description="Wybierz klienta CRM, usługę, eksperta i dostępny termin."
        ...
        <form id="create-appointment-form" class="form-stack" @submit.prevent="createStaffAppointment">
          ...
          <section class="appointment-booking-section">
            <div class="appointment-booking-section__heading">
              <span>2</span>
              <div>
                <h3>Usługa i ekspert</h3>
                <p>Dostępność uwzględnia godziny placówki, grafik eksperta i kalendarze zewnętrzne.</p>
              </div>
            </div>
            <div class="form-grid form-grid--three">
              <UFormField name="appointmentService" label="Usługa" required>
                <USelect
                  v-model="staffAppointmentForm.serviceId"
                  class="w-full"
                  :items="staffAppointmentServiceItems"
                  value-key="value"
    ...
    <UModal
      v-model:open="createServiceOpen"
      title="Dodaj usługę do placówki"
      description="Wybierz usługę z katalogu lub utwórz nową."
      :ui="{ content: 'sm:max-w-3xl', footer: 'justify-end' }"
    >
      <template #body>
        <form id="create-service-form" class="form-stack" @submit.prevent="createService">
          <UFormField name="serviceCatalog" label="Źródło usługi">
            <USelect
              v-model="serviceForm.existingServiceId"
          ...
          <template v-if="!serviceForm.existingServiceId && isOrganizationAdmin">
            <div class="form-grid form-grid--two">
              <UFormField name="newServiceName" label="Nazwa" required>
                <UInput v-model="serviceForm.name" class="w-full" />
              </UFormField>
              <UFormField name="newServiceDuration" label="Czas trwania (min)">
                <UInputNumber v-model="serviceForm.durationMinutes" :min="5" :max="1440" class="w-full" />
              </UFormField>
            </div>
            <UFormField name="newServiceDescription" label="Opis">
              <UTextarea v-model="serviceForm.description" class="w-full" :rows="2" />
            </UFormField>
            <div class="number-grid">
              <UFormField name="newServiceSlot" label="Interwał slotów">
                <UInputNumber v-model="serviceForm.slotIntervalMinutes" :min="5" :max="240" />
              </UFormField>
              <UFormField name="newServiceBefore" label="Bufor przed">
                <UInputNumber v-model="serviceForm.bufferBeforeMinutes" :min="0" :max="240" />
              </UFormField>
              <UFormField name="newServiceAfter" label="Bufor po">
                <UInputNumber v-model="serviceForm.bufferAfterMinutes" :min="0" :max="240" />
              </UFormField>
              <UFormField name="newServiceNotice" label="Wyprzedzenie min. (min)">
                <UInputNumber v-model="serviceForm.minNoticeMinutes" :min="0" :max="10080" />
              </UFormField>
              <UFormField name="newServiceAdvance" label="Horyzont (dni)">
                <UInputNumber v-model="serviceForm.maxAdvanceDays" :min="1" :max="730" />
              </UFormField>
            </div>
          </template>
          <UAlert
            v-else-if="!serviceForm.existingServiceId"
            color="neutral"
            variant="subtle"
            icon="i-lucide-lock-keyhole"
            title="Wybierz usługę z katalogu"
            description="Nowe definicje usług tworzy administrator organizacji. Administrator placówki może udostępnić istniejącą usługę."
          />
          ...
          <div class="assignment-block">
            ...
            <div v-if="bookableExperts.length" class="checkbox-grid">
              <UCheckbox
                v-for="expert in bookableExperts"
                :key="expert.user_id"
                :model-value="isIdSelected(serviceForm.expertUserIds, expert.user_id)"
                :label="facilityMemberLabel(expert)"
                @update:model-value="value => setIdSelected(serviceForm.expertUserIds, expert.user_id, value)"
              />
            </div>
            <p v-else class="empty-line">Brak ekspertów przyjmujących rezerwacje.</p>
          </div>
      ...
      <template #footer="{ close }">
        ...
        <UButton
          ...
          icon="i-lucide-plus"
          :disabled="!serviceForm.existingServiceId && (!isOrganizationAdmin || !serviceForm.name.trim())"
          :loading="savingService"
        >
          Dodaj usługę
        </UButton>
      ...
      <template #body>
        <form id="create-widget-form" class="form-stack" @submit.prevent="createWidget">
          ...
          <UFormField
            ...
            description="Widget eksperta pokazuje tylko jego usługi i wolne terminy. Tego zakresu nie można później zmienić."
          >
            <USelect v-model="widgetForm.fixedExpertUserId" :items="widgetExpertScopeItems" value-key="value" class="w-full" />
          </UFormField>
          <UAlert
            v-else
            color="neutral"
          ...
          <div class="assignment-block">
            <strong>Usługi w widgecie</strong>
            <div v-if="creatableWidgetServices.length" class="checkbox-grid">
              <UCheckbox
                v-for="service in creatableWidgetServices"
                :key="service.id"
                :model-value="isIdSelected(widgetForm.serviceIds, service.id)"
                :label="service.name"
                @update:model-value="value => setIdSelected(widgetForm.serviceIds, service.id, value)"
              />
            </div>
            <p v-else class="empty-line">Brak aktywnych usług w placówce.</p>
          </div>
        </form>
      </template>
      <template #footer="{ close }">
...
.period-row {
  display: grid;
  grid-template-columns: 120px auto 120px 34px;
  gap: 8px;
  align-items: center;
}
