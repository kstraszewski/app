...
const { data, pending, refresh } = await useFetch<{ data: Array<Record<string, any>>; count: number }>(() => crmApiPath('/clients'), {
...
async function createClient() {
  ...
  saving.value = true
  try {
    await $fetch(crmApiPath('/clients'), {
      method: 'POST',
      body: {
    ...
    <div class="clients-grid">
      <UCard>
        ...
        <div v-else-if="data.data.length" class="client-list">
          <NuxtLink v-for="client in data.data" :key="client.id" :to="orgPath(`/clients/${client.id}`)" class="client-row">
            <div>
