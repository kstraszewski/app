...
const { data, pending, refresh } = await useFetch<{ data: Record<string, any> }>(
  () => crmApiPath(`/cases/${caseId.value}`),
  { default: () => ({ data: { items: [], client: {}, activities: [], tasks: [], documents: [], properties: [] } }) },
)

const { data: productTypes } = await useFetch<{ data: Array<Record<string, any>> }>(() => crmApiPath('/product-types'), {
  default: () => ({ data: [] }),
})
const { data: providers } = await useFetch<{ data: Array<Record<string, any>> }>(() => crmApiPath('/providers'), {
  default: () => ({ data: [] }),
})
...
async function createSubmission() {
  ...
  try {
    await $fetch(crmApiPath(`/items/${submissionForm.case_item_id}/submissions`), {
      method: 'POST',
      body: {
...
async function updateItemStatus(item: Record<string, any>, status: string) {
  await $fetch(crmApiPath(`/items/${item.id}/status`), {
    method: 'PATCH',
...
<template>
  <CrmShell :title="data.data?.title || 'Sprawa'" eyebrow="Karta sprawy">
    <template #actions>
      <UButton :to="data.data?.client?.id ? orgPath(`/clients/${data.data.client.id}`) : orgPath('/clients')" icon="i-lucide-user" variant="outline">
        Klient
      </UButton>
      <UButton :to="orgPath('/cases')" icon="i-lucide-arrow-left" variant="outline">
        Sprawy
      </UButton>
    </template>
