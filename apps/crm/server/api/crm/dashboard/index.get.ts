...
export default defineEventHandler(async (event) => {
  ...
  const [
    ...
    session.supabase.from('crm_clients').select('id, display_name, status_code, updated_at').order('updated_at', { ascending: false }).limit(8),
    session.supabase.from('crm_cases').select('id, client_id, title, status_code, priority, updated_at').order('updated_at', { ascending: false }).limit(20),
    session.supabase.from('crm_case_items').select('id, case_id, title, status_code, amount_value, currency, updated_at').order('updated_at', { ascending: false }).limit(20),
    session.supabase.from('crm_tasks').select('*').neq('status_code', 'done').lte('due_at', endOfToday.toISOString()).order('due_at', { ascending: true }).limit(12),
    session.supabase.from('crm_case_item_settlements').select('status_code, expected_amount, due_amount, paid_amount, currency'),
    session.supabase.from('crm_item_submissions').select('status_code').limit(500),
  ])
