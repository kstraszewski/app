...
const { data: cases, pending, refresh } = await useFetch<{ data: Array<Record<string, any>>; count: number }>('/api/crm/cases', {
  query: computed(() => ({ q: search.value || undefined })),
  default: () => ({ data: [], count: 0 }),
})

const { data: clients } = await useFetch<{ data: Array<Record<string, any>> }>('/api/crm/clients', {
  default: () => ({ data: [] }),
})
    ...
    <div class="cases-grid">
      <UCard>
        ...
        <div v-else-if="cases.data.length" class="case-list">
          <NuxtLink v-for="item in cases.data" :key="item.id" :to="`/cases/${item.id}`" class="case-row">
            <div>
