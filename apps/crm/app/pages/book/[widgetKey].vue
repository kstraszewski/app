...
const weekDateKeys = computed(() => (
  weekStartDate.value ? isoDateRange(weekStartDate.value, BOOKING_WEEK_DAYS) : []
))
const slotsByDate = computed(() => {
  const result = new Map<string, PublicBookingSlot[]>()
  for (const date of weekDateKeys.value) result.set(date, [])
  for (const slot of slots.value) {
    const date = isoDateForTimestamp(slot.startsAt, slotsTimezone.value)
    const daySlots = result.get(date)
...
const weekDays = computed(() => weekDateKeys.value.map(date => ({
  date,
  weekday: shortWeekday(date),
  dateLabel: shortDayMonth(date),
  ariaLabel: fullDate(date),
  slots: slotsByDate.value.get(date) ?? [],
})))
const weekRangeLabel = computed(() => formatWeekRange(
  weekDateKeys.value[0] ?? '',
  weekDateKeys.value.at(-1) ?? '',
))
...
function shortDayMonth(value: string) {
  return new Intl.DateTimeFormat('pl-PL', {
    day: 'numeric',
    month: 'short',
    timeZone: 'UTC',
...
function formatWeekRange(start: string, end: string) {
  if (!start || !end) return 'Najbliższe 7 dni'
  ...
  const dateWithYearFormatter = new Intl.DateTimeFormat('pl-PL', {
    day: 'numeric',
    month: 'long',
    year: 'numeric',
    timeZone: 'UTC',
  ...
  if (startDate.getUTCFullYear() !== endDate.getUTCFullYear()) {
    return `${dateWithYearFormatter.format(startDate)} – ${dateWithYearFormatter.format(endDate)}`
  }
  return `${dateFormatter.format(startDate)} – ${dateWithYearFormatter.format(endDate)}`
}
...
.booking-consent__warning {
  color: var(--ui-text-error) !important;
}
