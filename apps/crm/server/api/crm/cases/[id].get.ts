...
export default defineEventHandler(async (event) => {
  ...
  const [
    ...
    session.supabase.from('crm_clients').select('*').eq('id', caseRow.client_id).single(),
    session.supabase.from('crm_client_people').select('*').eq('client_id', caseRow.client_id).order('created_at'),
    session.supabase.from('crm_case_participants').select('*').eq('case_id', id),
    session.supabase.from('crm_case_items').select('*').eq('case_id', id).order('updated_at', { ascending: false }),
    ...
    session.supabase.from('crm_tasks').select('*').eq('case_id', id).order('due_at', { ascending: true }),
    session.supabase.from('crm_documents').select('*').eq('case_id', id).order('created_at', { ascending: false }),
    session.supabase.from('crm_properties').select('*').eq('case_id', id).order('created_at', { ascending: false }),
    session.supabase.from('crm_activities').select('*').eq('case_id', id).order('created_at', { ascending: false }).limit(80),
  ])
