...
import type { PresetName, PresetTheme } from '../presets';
...
export type RenderKey = `${PresetName}:${PresetTheme}`;
...
export interface MetalFxInstance {
  ...
  everCopied: boolean;
  /** A registered SVG glow needs one sample from the instance's current
   *  render key. Kept separate from `everCopied` because one shared frame can
   *  copy several paused canvases while the readback budget updates only one
   *  glow candidate at a time. */
  glowDirty: boolean;
  dpr: number;
...
export interface SharedRenderer {
  ...
  /** The preset uniforms currently uploaded to the shared GL program. */
  uploadedRenderKey: RenderKey | null;
  contextLost: boolean;
  ...
  glowIdx: number;
  glowSkip: number;
  /** A readback cache per preset/theme, so glow never samples another group. */
  glowFrames: Map<RenderKey, GlowFrame>;
}
...
export function ensureSharedRenderer(): SharedRenderer {
  ...
  const onContextRestored = () => {
    ...
    SHARED.uploadedRenderKey = null;
    SHARED.glowFrames.clear();
