...
export function buildOrganizationDesignCss(value: unknown): string {
  ...
  const base: Record<string, string> = {
    ...
    '--oe-motion-fast': `${settings.motion.fast}ms var(--ease-oe)`,
    '--oe-motion-base': `${settings.motion.base}ms var(--ease-oe)`,
    '--oe-motion-slow': `${settings.motion.slow}ms var(--ease-oe)`,
