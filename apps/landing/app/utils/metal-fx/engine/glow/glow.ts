...
export function updateGlow(h: GlowHandles, inst: MetalFxInstance, nowMs: number, strengthMul: number, theme: 'dark' | 'light' = 'dark'): void {
  ...
  if (!h.relocTween || h.relocTween.done) {
    if (h.appearedAt === 0) {
      ...
      h.wanderS = 0; h.wanderTargetS = 0; h.wanderFrames = 0;
      const np = perim[h.currentIdx] ?? perim[0]!;
  ...
  const blobArc = (perim[h.currentIdx] ?? perim[0]!).arc + h.wanderS;
  ...
  // A preset/theme switch creates a new render key. Do not carry the dark
  // mode tint-hold window across that boundary: the metal ring and its halo
  // must communicate the same preset in the same rendered frame.
  if (h.lastRenderKey !== inst.renderKey) {
    h.lastRenderKey = inst.renderKey;
    h.tintTween = null;
    h.tintHoldUntil = 0;
    h.lastHaloStroke = '';
    h.lastExtraStroke = '';
