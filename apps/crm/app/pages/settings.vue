...
const { data: productTypes, refresh: refreshProductTypes } = await useFetch<{ data: Array<Record<string, any>> }>('/api/crm/product-types', {
  default: () => ({ data: [] }),
})
const { data: workflows } = await useFetch<{ data: Array<Record<string, any>> }>('/api/crm/workflows', {
  default: () => ({ data: [] }),
})
const { data: providers, refresh: refreshProviders } = await useFetch<{ data: Array<Record<string, any>> }>('/api/crm/providers', {
  default: () => ({ data: [] }),
})
...
async function createProductType() {
  ...
  try {
    await $fetch('/api/crm/product-types', {
      method: 'POST',
      body: { ...productForm },
    })
    productForm.name = ''
