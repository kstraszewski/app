...
const { data, pending, refresh } = await useFetch<{ data: Record<string, any>; people: any[]; cases: any[]; activities: any[] }>(
  () => `/api/crm/clients/${clientId.value}`,
...
<template>
  <CrmShell :title="data.data?.display_name || 'Klient'" eyebrow="Karta klienta">
    <template #actions>
      <UButton to="/clients" icon="i-lucide-arrow-left" variant="outline">
        Klienci
      </UButton>
    </template>
      ...
      <div class="stack">
        ...
        <UCard>
          ...
          <div v-if="data.cases.length" class="case-list">
            <NuxtLink v-for="item in data.cases" :key="item.id" :to="`/cases/${item.id}`" class="case-row">
              <div>
